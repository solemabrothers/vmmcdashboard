...
class Controller extends BaseController
{
    ...
    $start_cop_year='2020-10-01';
        $numbersHIVnegative=DB::select("SELECT SUM(c.NumberHIVNegative) as negative FROM mets_vmmc.circumcision c   where c.SummaryDate >='$start_cop_year' ");
        $numbersHIVpositive =DB::select("SELECT SUM(c.NumberHIVPositive) as positive FROM mets_vmmc.circumcision c  where c.SummaryDate >= '$start_cop_year' ");
 ...
 +SUM(c.NumberMildSwellingHaematoma)
  ...
  FROM mets_vmmc.circumcision c   where c.SummaryDate >= '$start_cop_year'");
...
+SUM(c.NumberSevereInfection)+SUM(c.NumberSeverePain)AS Severe,
    ...
    +SUM(c.NumberMildPain)As ClientsAffected
      ...
      AND c.SummaryDate >= '$start_cop_year'   group by facility,implementingpartner
...
FROM mets_vmmc.circumcision c,mets_vmmc.facility f , mets_vmmc.implementingpartner Ip, mets_vmmc.district d
 WHERE  c.Facility=f.Facility AND c.ImplementingPartner= Ip.IP_ID  AND f.district_id= d.district_id
  AND c.SummaryDate >= '$start_cop_year'  group by c.Facility,c.ImplementingPartner
  HAVING positive !=0");
...
where c.SummaryDate >= '$start_cop_year' AND c.ImplementingPartner!=13
group by IP_ID");
...
where c.SummaryDate >= '$start_cop_year' ");
 ...
 $facilitiesusingdevices=DB::select("SELECT Ip.Ip_name,d.District_name,f.facility_name,f.Facility,SUM(c.NumberDeviceType) AS DevicesUsed
      FROM mets_vmmc.circumcision c,mets_vmmc.implementingpartner Ip,mets_vmmc.district d, mets_vmmc.facility f WHERE c.ImplementingPartner=Ip.IP_ID AND c.Facility=f.Facility AND f.district_id=d.district_id
      AND c.SummaryDate >= '$start_cop_year'  group by facility,implementingpartner HAVING DevicesUsed !=0");

        $totalperformance =number_format(($totalnumbercircumscised[0]->total/$totaltarget[0]->target)*100,2,'.','');
       return view('layouts.home', compact('districts','ips','totalperformance','totaltarget','totalnumbercircumscised','adverseeventsbyfacility','facilitiesusingdevices','SeverelyAffected','monthly_data','numbersHIVnegative','clientsHIVPositivebyfacility','numbersHIVpositive','totalnumbercircumscisedusingdevices'));
    }
