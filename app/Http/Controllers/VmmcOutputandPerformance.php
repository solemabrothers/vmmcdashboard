...
AS totalperformance FROM mets_vmmc.circumcision c,mets_vmmc.implementingpartner Ip,mets_vmmc.district d, mets_vmmc.facility f WHERE c.ImplementingPartner=Ip.IP_ID AND c.Facility=f.Facility AND f.district_id=d.district_id
      AND c.SummaryDate between '$start_cop_year' and '$end_cop_year' group by implementingpartner,d.district_id");
...
AS totalperformance FROM mets_vmmc.circumcision c,mets_vmmc.implementingpartner Ip,mets_vmmc.district d, mets_vmmc.facility f WHERE c.ImplementingPartner=Ip.IP_ID AND c.Facility=f.Facility AND f.district_id=d.district_id
      AND c.SummaryDate >= '$start_cope_year'  group by implementingpartner,d.district_id");
        ...
        $facilityperformance=DB::select("SELECT d.district_id,facility_name, SUM(NumberCircumcised) as facilitydata from mets_vmmc.circumcision c
                                        ...
                                        WHERE c.SummaryDate >='$start_cope_year'
...
where c.SummaryDate >='$start_cope_year' AND t.Year_of_target=\"2021\" group by ImplementingPartner");
    ...
    inner join implementingpartner i on ft.ImplementingPartner = i.IP_ID and ft.Year_of_Target=2021  group by ft.ImplementingPartner');
