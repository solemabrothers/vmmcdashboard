...
class FilterData extends BaseController
{
    public function index(Request $request)
    {
        ...
        {
            $ipmechanismname = DB::select("SELECT implementingpartner.Ip_name from implementingpartner where implementingpartner.IP_ID='$Ip_ID'");
            $Ip_mechanismachievement =DB::select("SELECT SUM(c.NumberCircumcised) As total FROM mets_vmmc.circumcision c WHERE c.ImplementingPartner='$Ip_ID' AND  DATE(c.SummaryDate) BETWEEN '$startDate' AND '$endDate' ");
            $IP_target =DB::select("SELECT SUM(Total) As IpMechanismtarget from facilitytargets where ImplementingPartner = '$Ip_ID'");
            $devicesusedbyip=DB::select("select SUM(NumberDeviceType) as DevicesUsed from circumcision c where c.ImplementingPartner=ImplementingPartner = '$Ip_ID'  AND  DATE(c.SummaryDate) BETWEEN '$startDate' AND '$endDate' ");
         ...
         WHERE c.ImplementingPartner='$Ip_ID'  AND DATE(c.SummaryDate)  AND  DATE(c.SummaryDate) BETWEEN '$startDate' AND '$endDate' group by f.facility
         HAVING  ClientsAffected !=0 OR Severe!=0");
            ...
            WHERE   c.Facility = f.Facility AND f.district_id = d.District_ID AND c.ImplementingPartner='$Ip_ID' AND  DATE(c.SummaryDate) BETWEEN '$startDate' AND '$endDate'
GROUP BY f.district_id");
      ...
      FROM mets_vmmc.circumcision c,mets_vmmc.implementingpartner Ip,mets_vmmc.district d, mets_vmmc.facility f WHERE c.ImplementingPartner=3 AND c.Facility=f.Facility AND f.district_id=d.district_id
       AND  DATE(c.SummaryDate) BETWEEN '$startDate' AND '$endDate' group by facility,implementingpartner HAVING DevicesUsed !=0 ");
            ...
            $ipagecategoryBetween10And14 = DB::table('circumcision')->whereBetween('SummaryDate',[$startDate,$endDate])->whereRaw("ImplementingPartner='$Ip_ID'")-> sum('NumberCircumcisedBetween10And14');
            $ipagecategoryBetween15And19 = DB::table('circumcision')->whereBetween('SummaryDate',[$startDate,$endDate])->whereRaw("ImplementingPartner='$Ip_ID'")-> sum('NumberCircumcisedBetween15And19');
            $ipagecategoryBetween20And24 = DB::table('circumcision')->whereBetween('SummaryDate',[$startDate,$endDate])->whereRaw("ImplementingPartner='$Ip_ID'")-> sum('NumberCircumcisedBetween20And24');
            $ipagecategoryBetween26And30 = DB::table('circumcision')->whereBetween('SummaryDate',[$startDate,$endDate])->whereRaw("ImplementingPartner='$Ip_ID'")-> sum('NumberCircumcisedBetween25And29');
            $ipagecategoryBetween31And34= DB::table('circumcision')->whereBetween('SummaryDate',[$startDate,$endDate])->whereRaw("ImplementingPartner='$Ip_ID'")-> sum('NumberCircumcisedBetween30And34');
            $ipagecategoryBetween35And39= DB::table('circumcision')->whereBetween('SummaryDate',[$startDate,$endDate])->whereRaw("ImplementingPartner='$Ip_ID'")-> sum('NumberCircumcisedBetween35And39');
            $ipagecategoryBetween40And44= DB::table('circumcision')->whereBetween('SummaryDate',[$startDate,$endDate])->whereRaw("ImplementingPartner='$Ip_ID'")-> sum('NumberCircumcisedBetween40And44');
            $ipagecategoryBetween45And49= DB::table('circumcision')->whereBetween('SummaryDate',[$startDate,$endDate])->whereRaw("ImplementingPartner='$Ip_ID'")-> sum('NumberCircumcisedBetween45And49');
            $Above50= DB::table('circumcision')->whereBetween('SummaryDate',[$startDate,$endDate])->whereRaw("ImplementingPartner='$Ip_ID'")->value(DB::raw("SUM(NumberCircumcisedBetween50And54+NumberCircumcisedBetween55And59+NumberCircumcised60andabove)"));
            ...
            $facilityoutputperformance=DB::select("SELECT AB.District_name,AC.Achievement, AB.Target,ROUND((AC.Achievement/(AB.Target)*100),2) As Performance from
(SELECT d.district_id, d.District_name,SUM(ft.Total) Target  from facilitytargets ft,district d where ft.ImplementingPartner ='$Ip_ID' and d.district_id=ft.district_id group by d.district_id) AB
INNER JOIN (SELECT  d.district_id,d.District_name,
            SUM(c.NumberCircumcised) As Achievement
            FROM mets_vmmc.circumcision c,facility f,district d
            WHERE   c.Facility = f.Facility AND f.district_id = d.District_ID AND c.ImplementingPartner='$Ip_ID' AND  DATE(c.SummaryDate)>='2019-10-01' GROUP BY f.district_id) AC
ON AC.district_id=AB.district_id");
        }
        else {
...
");
            ...
            $facilityoutputperformance=DB::Select("SELECT AC.facility_name,AB.Target,AC.Achievement,ROUND((AC.Achievement/(AB.Target)*100),2)  As Performance from
(SELECT f.Facility, f.facility_name as District_name ,ft.Total As Target FROM mets_vmmc.facilitytargets ft,facility f,district d
            WHERE   ft.Facility = f.Facility AND f.district_id = d.District_ID AND d.district_id='$districts_ID')AB
INNER JOIN(SELECT  f.Facility, f.facility_name,SUM(c.NumberCircumcised) as Achievement,ft.Total As Target,ROUND((sum(NumberCircumcised)/ft.Total*100),2) As Performance
FROM mets_vmmc.facilitytargets ft,mets_vmmc.circumcision c ,facility f,district d
WHERE  c.Facility=ft.Facility and  ft.Facility = f.Facility and c.SummaryDate>'2019-10-01' AND f.district_id = d.District_ID AND d.district_id='$districts_ID' group by f.Facility) AC
ON AC.Facility=AB.Facility");
